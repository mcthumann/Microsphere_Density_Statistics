# -*- coding: utf-8 -*-
"""
Created on Mon Sep 30 12:56:18 2024
...
class Const:
    ...
    runs = 5000
    output_file_name = "just_mass_5_to_7.txt"

def get_VACF_from_VPSD(times, full_mass, gamma_s, tau_f, integ_points):
    frequencies = np.logspace(3, 9, integ_points)
...
def fit_to_ACF(times, measured_vacf, radius_mean, gamma_s, tau_f, integ_points):
    ...
    def vacf_fit_density(times, density):
        full_mass = (4 / 3) * math.pi * (radius_mean) ** 3 * density
...
if __name__ == "__main__":
    ...
    output = open(Const.output_file_name, "w")
    output.write("Running average, density measurements \n")
    output.close()
    ...
    for i in range(Const.runs):
        ...
        rolling_mean.append(np.mean(fit_densities))
        
        output.write(str(rolling_mean[i]) + ", " +str(fit_densities[i]) + "\n")
        output.close()
    ...
    plt.axhline(density_mean + 500, color='r', linestyle='--', label=f'Real Mean = {density_mean}')
